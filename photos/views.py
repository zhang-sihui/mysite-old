...
def display_photo(request):
    db_photo_list = Photo.objects.filter(pub_date__lte=timezone.now())
    ...
    return render(request, 'photo/display.html', {'page': number, 'paginator': paginator})
...
def download(request, photo_id):
    ...
    photos_path = os.path.join(base_dir, 'photos', 'images', 'modified', photo_name)  # 获取图片
